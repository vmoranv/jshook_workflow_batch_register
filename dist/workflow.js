 ...
 */
import { branchNode, parallelNode, sequenceNode, toolNode, } from '@jshookmcp/extension-sdk/workflow';
const batchRegisterWorkflow = {
    kind: 'workflow-contract',
    version: 1,
    id: 'workflow.batch-register.v1',
    displayName: 'Batch Register Accounts',
    description: 'Run register_account_flow for multiple accounts with concurrency controls, ' +
        'retry policies, and success rate gating.',
    tags: ['workflow', 'registration', 'batch', 'automation'],
    timeoutMs: 15 * 60_000,
    defaultMaxConcurrency: 3,
    build(ctx) {
        const maxConcurrency = ctx.getConfig('workflows.batchRegister.maxConcurrency', 3);
        ...
        const registerSteps = parallelNode('register-parallel', [
            toolNode('register-account-1', 'register_account_flow', {
                input: {
            ...
            toolNode('register-account-2', 'register_account_flow', {
                input: {
                    registerUrl: 'https://example.com/register',
        ...
        const summarize = branchNode('summary-branch', 'batch_success_rate_gte_80', toolNode('success-summary', 'console_execute', {
